...
Resultado: el **kernel integrado alcanza el techo de CartPole** (greedy ~500/500) —las facultades no rompen el control— y **Ψ se mantiene vivo** como señal de integración sobre el estado recurrente (sube con el aprendizaje). Es la respuesta constructiva a §3.10: el kernel de inferencia activa, **dotado de un world-model recurrente entrenado sobre secuencias**, resuelve la tarea externa manteniendo su ciclo de consciencia. (Es una composición de referencia; la fusión in-situ está en §3.12.)

### 3.12 Fusión in-situ: el `ConsciousKernel` canónico con world-model RSSM (`exp_kernel_rssm.py`)
La culminación: la **misma clase** `ConsciousKernel`, con `world_model_type="rssm"`, corre su **step-loop completo** (`PERCIBIR→…→SOÑAR`) y `_compute_psi` —el mismo método de Ψ que el path GRU— sobre un RSSM (recurrente, entrenado sobre secuencias, reward aprendido) en lugar de su world-model de un paso. La rama RSSM redimensiona las facultades al espacio de features `s=[h,z]` y delega percepción/control al `DreamerV3Agent`; el aprendizaje (replay de secuencias + actor-crítico en imaginación) ocurre vía `learn_rssm()` tras cada paso. El path GRU por defecto queda **byte-idéntico** (los 504 tests del kernel intactos).
...
|---|---|
| random | −1189 |
| **kernel fusionado (greedy)** | **−181 ± 37** |
| (referencia: swing-up resuelto) | > −250 |

El kernel **RESUELVE el control continuo** (greedy −181, ambas semillas bajo el umbral; el retorno de entrenamiento sube de −1365 a −149 hacia el episodio ~13 y se mantiene). Dos cosas lo destrabaron sobre un intento parcial previo (−545): (i) **normalización del retorno con baseline** en el objetivo del actor por gradientes de valor —desacopla el gradiente de la escala grande del reward de Pendulum (~[−16,0])—, y (ii) **action repeat = 2** (estándar en Pendulum/DMC). Al converger limpio, **Ψ ahora sí consolida** (0.02→0.94), a diferencia del intento parcial: el índice de integración sube fuerte cuando el agente domina la tarea. El mismo `ConsciousKernel` resuelve control **discreto** (CartPole, techo) y **continuo** (Pendulum) con su ciclo de consciencia intacto.

### 3.14 Continuo de mayor dimensión: MuJoCo Reacher-v5 (`exp_mujoco.py`)
Extiende el control continuo a una tarea **MuJoCo de mayor dimensión**: Reacher-v5 (gymnasium MuJoCo) — brazo de 2 articulaciones, **acción 2-D**, obs 10-D, reward denso (−distancia a un target aleatorio − costo de control). El mismo kernel fusionado lo controla (obs estandarizadas por un rollout aleatorio).

| política | retorno de episodio |
|---|---|
| random | −43 |
| **kernel fusionado (greedy)** | **−20** |
| (referencia: óptimo) | ~−5/−10 |

El kernel **aprende** la tarea (greedy −20, ~2× sobre random; el retorno sube de ~−75 a ~−20 y se estabiliza), **extendiendo el control continuo a MuJoCo de mayor dimensión** (2-D acción, 10-D obs). No alcanza el óptimo: nuestro entrenamiento es **CPU-bound** y las tareas MuJoCo piden mucho más presupuesto (GPU). Aquí Ψ sube a ~0.5 a media marcha pero **no consolida** (decae a ~0.05): el target **aleatorio por episodio** mantiene el error de predicción alto, así que precisión/coherencia no se asientan — consistente con el patrón "Ψ consolida solo cuando la tarea se domina". Nota de instalación: **dm_control 1.0.41 choca con mujoco 3.9** (`flex_bandwidth`), así que usamos la suite MuJoCo de gymnasium — el **mismo motor MuJoCo**.
...
| Control continuo (Pendulum) | **Sí** | actor tanh-Gaussian por gradientes de valor (con normalización de retorno + action-repeat) **resuelve** el swing-up: greedy −181 vs random −1189; Ψ consolida 0.02→0.94 |
| Continuo de mayor dim (MuJoCo Reacher) | **Parcial** | aprende greedy −20 vs random −43 (2-D acción / 10-D obs); no óptimo en presupuesto CPU; Ψ no consolida (target aleatorio) |
...
- Control continuo **resuelto** en Pendulum (§3.13) y **aprendido** en MuJoCo
  Reacher (§3.14, mayor dimensión). Límites honestos: (a) entrenamiento
  **CPU-bound** → tareas MuJoCo ricas (locomoción Hopper/HalfCheetah, ~1e6 pasos)
  fuera de alcance, solo parciales; (b) **dm_control** propio choca con la versión
  de mujoco instalada — usamos su mismo motor vía gymnasium. Falta GPU + más
  presupuesto para tareas MuJoCo de locomoción.
...
## 6. Conclusión
El Conscious Kernel es un sustrato de inferencia activa coherente y honesto que **aprende** (predicción), **controla** (EFE continuo y un actor amortizado estilo Dreamer que iguala/supera a la búsqueda a costo O(1); con su world-model de un paso transfiere parcialmente a CartPole, y **con un world-model RSSM fusionado in-situ (`world_model_type="rssm"`) alcanza el techo manteniendo Ψ vivo**, §3.12), **integra** (Ψ robusto) y tiene un camino para **acoplarse a un agente vivo** (Yvyra). El aporte metodológico es tanto el sistema como la disciplina de falsación: incluyendo el desmontaje del propio claim que dio nombre al proyecto y la **retractación de un resultado propio** (la curiosidad) que una revisión adversarial mostró confundido por RNG.
