...
4. 🟡 **PARCIAL — Benchmark externo de control** (CartPole-v1, `exp_cartpole.py`): el kernel transfiere parcialmente (greedy 177 vs random 22; 35% del óptimo) como regulación de inferencia activa. Se añadieron estabilizadores estilo DreamerV3 (target critic EMA, normalización de returns, grad clip) — ayudan poco; el actor-crítico online sobre buffer no-estacionario **sigue oscilando**. Falta el loop DreamerV3 completo (replay de transiciones + reentreno del world model) y DMC/continuo.
