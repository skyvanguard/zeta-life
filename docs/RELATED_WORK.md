...
2. ✅ **HECHO — Marco de indicator properties de Butlin** (`docs/INDICATOR_PROPERTIES.md`): auditoría honesta y conservadora del kernel contra las 14 propiedades — fuerte en PP/agencia/embodiment (PP-1, AE-1, AE-2), parcial en GWT/recurrencia/HOT (8), ausente AST-1/HOT-4/GWT-4. Reemplaza el uso de Ψ como pseudo-medida de consciencia, sin afirmar que el kernel sea consciente.
3. ✅ **HECHO — Ensemble de dinámica real** (`dynamics_ensemble`, `exp_curiosity.py`): miembros independientes `(latente,acción)→obs` (no cabezas de readout compartido). Al construirlo descubrimos que el null de curiosidad previo estaba **subponderado**: con `efe_epistemic_weight` conmensurable, la curiosidad por disagreement **sí impulsa exploración** (pareado +0.30, t=3.7, 9/10). El ensemble independiente es la señal más principista (consistente en el barrido de pesos). Corrige la conclusión previa en ambas direcciones.
4. ✅ **HECHO (acotado) — Benchmark externo de control** (CartPole-v1): el kernel (world-model de 1 paso) transfiere parcialmente (~33% del óptimo) y el replay de transiciones mejora la curva pero no cierra la brecha. Para **acotar el límite** construimos un agente **DreamerV3-style con RSSM** (`kernel/rssm.py`, `kernel/dreamerv3_agent.py`, `exp_dreamerv3.py`): estado recurrente entrenado sobre **secuencias** + **reward model** aprendido. **Resuelve CartPole al techo (greedy 500/500)** → la brecha del kernel es su world-model de un paso, **no** la inferencia activa. **Integración hecha** — composición (`kernel/rssm_kernel.py`, §3.11) **y fusión in-situ** en la clase canónica (`ConsciousKernel(world_model_type="rssm")`, `kernel/rssm_kernel`+`conscious_kernel.py`, `exp_kernel_rssm.py`, §3.12): el mismo `step()`/`_compute_psi` corre sobre el RSSM y alcanza el techo de CartPole con Ψ vivo (path GRU byte-idéntico). **Control continuo** extendido y **resuelto** (Pendulum-v1, `exp_pendulum.py`, §3.13): actor tanh-Gaussian por gradientes de valor (con normalización de retorno + action-repeat); el kernel fusionado resuelve el swing-up (greedy −181 vs random −1189), Ψ consolida 0.02→0.94. Pendiente: DMC propio (MuJoCo) y entornos continuos más ricos.
